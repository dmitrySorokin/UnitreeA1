...
class Critic(Module):
    def __init__(self, state_dim, arch, action_dim, n_quantiles, n_nets):
        super().__init__()
        self.nets = []
        self.n_quantiles = n_quantiles
        self.n_nets = n_nets
        for i in range(n_nets):
            net = Mlp(state_dim + action_dim, arch, n_quantiles)
...
class Actor(Module):
    def __init__(self, state_dim, arch, action_dim):
        super().__init__()
        self.action_dim = action_dim
        self.net = Mlp(state_dim, arch, 2 * action_dim)
